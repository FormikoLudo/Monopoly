...
public class CardView extends Group {
    ...
    private void drawDomain(Batch batch, float parentAlpha) {
        ...
        shapeDrawer.rectangle(new Rectangle(getX() + 10f, getY() + 10f,
                getWidth() * .9f, getHeight() * .9f),
                Color.BLACK);
        BackgroudColoredActor bgactor = new BackgroudColoredActor(
                getWidth() * .9f, getHeight() * .2f, Color.BLUE, shapeDrawer);
        bgactor.setPosition(getX() + 10f, getY() + .75f * getHeight());
        bgactor.draw(batch, parentAlpha);

        FONT.draw(batch, "TITRE DE PROPRIÉTÉ", getX() + 15f,
                getY() + .9f * getHeight());
        REGULAR_FONT.setColor(Color.WHITE);
        PROPERTY_NAME_FONT.draw(batch, model.getName(), getX() + 15f,
                getY() + .85f * getHeight());
        ...
        shapeDrawer.line(getX() + 15f, getY() + .30f * getHeight(),
                getX() + 0.9f * this.getWidth(),
                getY() + .30f * getHeight(), Color.BLACK, 2f);
        REGULAR_FONT.draw(batch, "Prix des maisons", getX() + 15f,
                getY() + .25f * getHeight());
        REGULAR_FONT.draw(batch, "Prix d'1 hôtel", getX() + 15f,
                getY() + .20f * getHeight());
        REGULAR_FONT.draw(batch, "(plus 4 maisons)", getX() + 15f,
                getY() + .15f * getHeight());
        ...
        batch.draw(textRegion, getX(), getY(), getX(), getY(), getWidth(),
                getHeight(), 1.f, 1.f, 45);
    }
    ...
    private void drawMortgagedView(Batch batch, float parentAlpha) {
        ...
        shapeDrawer.line(
                ...
                new Vector2(getX() + .93f * getWidth(), getY() + .4f * getHeight()));

        // WidgetsFactory.drawRotatedText(REGULAR_FONT,"MORGAGED",getX(),getY(),batch,parentAlpha);

        shapeDrawer.line(
                new Vector2(getX() + 10f, getY() + .4f * getHeight()),
                new Vector2(getX() + .9f * getWidth(), getY() + .2f * getHeight()));
    }

    public void drawHouse(Batch batch, float parentAlpha, float x, float y,
            boolean hotel) {
