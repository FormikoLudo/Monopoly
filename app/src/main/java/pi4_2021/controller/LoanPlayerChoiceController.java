 ...
 */
public class LoanPlayerChoiceController
{
  /**
  ...
  /**
   * The taker of the loan name.
   */
  @FXML private Label loaner;
  ...
  {
    this.model = model;
    loaner.setText(model.getCurrentPlayer().getName());

    //Creating an ArrayList of the players.
    ArrayList<Player> choices = new ArrayList<>();
    for(int i = 0; i < model.getPlayers().length; i++)
    {
      if(!(model.getPlayers()[i] instanceof BotPlayer) && model.getPlayers()[i].getStillPlaying())
      {
        choices.add(model.getPlayers()[i]);
      }
    }
    choices.remove(model.getCurrentPlayer());
  ...
  public void nextAction(ActionEvent event) throws IOException
  {
    ...
    else
    {
      ...
      FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/PlayerLoan.fxml"));
      ...
      PlayerLoanController plc = loader.getController();
      plc.init(model,((Player)players.getValue()));
